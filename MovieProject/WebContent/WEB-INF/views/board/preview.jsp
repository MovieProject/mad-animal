	...
	<table id="listtable" class="maintable" align="center">
		...
		<tbody>
			<c:choose>
				...
				<c:otherwise>
					<c:forEach var="board" items="${requestScope.boardList}" end="4"
						step="1">
						<tr>
							<td class="previewNum">${board.boardNum}</td>
							<td class="previewTitle"><c:forEach begin="1"
									end="${board.replyStep}">
						&nbsp;
					</c:forEach> <a
								href="read?pageNumber=${currentPageNumber}&boardNum=${board.boardNum}&searchType=${param.searchType}&searchText=${param.searchText}"
								target="_parent">${board.title}</a></td>
