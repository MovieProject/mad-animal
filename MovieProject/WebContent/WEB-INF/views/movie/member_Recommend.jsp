	...
	<div id="page">
		...
		<div id="content">
			<table id="listtable" class="maintable">
				...
				<thead>
					<tr><c:if test="${loginMember.grade eq 2 }"><th class = "check"></th></c:if>
						<th class="num"></th>
							...
							<c:forEach var="movie" items="${requestScope.movielist}">
								<tr>
									<c:if test="${loginMember.grade eq 2 }">
										<td class="check"><input type="checkbox" id="items" name="items" value="${movie.movieNum}"></td>
									</c:if>
				...
				<tfoot>
					<tr>
						...
						<c:if test="${startPageNumber >1}">
							<a href="movielist?type=1&pageNumber=${startPageNumber - 1}&searchType=${searchType}&searchText=${searchText}">이전</a>
						</c:if>
						<c:forEach begin="${startPageNumber}" end="${endPageNumber}" var="pageNumber">
							<c:choose>
								...
								<c:otherwise>
									<a class="pagenumber" href="movielist?type=1&pageNumber=${pageNumber}&searchType=${searchType}&searchText=${searchText}">${pageNumber}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${endPageNumber < totalPageCount}">
							<a href="movielist?type=1&pageNumber=${endPageNumber + 1}&searchType=${searchType}&searchText=${searchText}">다음</a>
						</c:if>
						</td>
					</tr>
				</tfoot>
