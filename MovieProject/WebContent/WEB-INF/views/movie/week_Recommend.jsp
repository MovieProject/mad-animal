	...
	<div id="page">
		...
		<div id="content">
			<table id="listtable" class="maintable">
 				<caption id="boardtitle">금주 추천 영화</caption>
				<thead>
					<tr>
						<th class="num"></th>
						...
						<c:otherwise>
							<c:forEach var="movie" items="${requestScope.movielist}">
								<tr>
									...
									<td class="title">									
									 <a	href="movieRead?pageNumber=${currentPageNumber}&num=${movie.movieNum}&searchType=${param.searchType}&searchText=${param.searchText}&type=2">${movie.movieName}</a></td>
					...
					<tr>
						<td id="pagenavigator" colspan="5">
							<c:if test="${startPageNumber >1}">
								<a href="movielist?type=2&pageNumber=${startPageNumber - 1}&searchType=${searchType}&searchText=${searchText}">이전</a>
							</c:if>
							<c:forEach begin="${startPageNumber}" end="${endPageNumber}" var="pageNumber">
								<c:choose>
									...
									<c:otherwise>
										<a class="pagenumber" href="movielist?type=2&pageNumber=${pageNumber}&searchType=${searchType}&searchText=${searchText}">${pageNumber}</a>
									</c:otherwise>
								</c:choose>
							</c:forEach>
								<c:if test="${endPageNumber < totalPageCount}">
								<a href="movielist?type=2&pageNumber=${endPageNumber + 1}&searchType=${searchType}&searchText=${searchText}">다음</a>
							</c:if>
						</td>
					</tr>
				</tfoot>
			...
			<div class="buttonbar" >
				<form name="searchForm" action="movielist" method="post" onsubmit="return searchCheck(this.form);">
					...
					<input type="button" value="목록" onclick="goUrl('movielist?type=2');">
